...
const findCreatedAbsenceRequests = async ({
  requestType,
  fromDate,
  toDate,
  status = [],
}) => {
  const rangeDateCondition = { $gte: new Date(fromDate) };
  if (toDate != null) rangeDateCondition.$lte = new Date(toDate);

  const result = await AbsenceRequest.aggregate([
    {
      $match: {
        requestType,
        status: { $in: status },
        $or: [
          {
            fromDate: rangeDateCondition,
          },
          {
            toDate: rangeDateCondition,
          },
        ],
      },
    },
